...
export class TelegramBotService {
  ...
  private async handleIncomingMessage(msg: TelegramMessage): Promise<void> {
    ...
    if ((textContent || imageUrls.length > 0 || filePaths.length > 0) && this.bot) {
      await this.processWithAgentAndReply(msg.chat.id, textContent, imageUrls, traceId)
    }
  }
   ...
   */
  private async processWithAgentAndReply(chatId: number, userMessage: string, imageUrls: string[] = [], traceId?: string): Promise<void> {
